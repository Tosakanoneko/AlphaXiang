...
class hand_detector:
    def __init__(self):
        ...
        parser.add_argument('--data', type=str, default='/home/aidlux/Chinese-chess-GHT2/detect_hand/data/category.data', 
               help='Specify training profile *.data')
        parser.add_argument('--weights', type=str, default='/home/aidlux/Chinese-chess-GHT2/detect_hand/weights/train5/hand-150-epoch-0.312856ap-model.pth', 
                            help='The path of the .pth model to be transformed')
    ...
    def run_hand_detect(self):
        model = detector.Detector(1, 3, True).to(self.device)
        model.load_state_dict(torch.load(self.opt.weights, map_location=self.device)) 
        model.eval()
        while True:

...
This project is forked from https://github.com/lvgl/lv_micropython and can be used to build and run a MicroPython based UI code exported from SquareLine Studio.
...
## Add the UI files
In SquareLine Studio click `Export->Export Files` and select the `port/<your-platform>` folder of this project.

## Build and run the project

In `ports/<your-platform>` create a new file called `main.py` with the following content
```py
from display_driver_utils import driver
drv = driver(width=800, height=480)
import ui
```

Replace 800 and 480 with the resolution of the UI.

After that build the MicroPython and LVGL. For the `ports/unix`:
```sh
make -j
```
For other ports see the instructions in `ports/<your-platform>/README.md`

Start MicroPython with
```sh
./micropython -X heapsize=512m 
```
It should open the MicroPython's REPL terminal.

Finally start he UI with type:
```py
import main
```
and the UI should appear.


_The rest of the README is the content of the original file._
---
